...
public class ConveyorEntity : MachineEntity, PowerConsumerInterface, ItemSupplierInterface, ItemConsumerInterface
{
    ...
    float mrSleepTimer;
    public int autoturncheck;
    public int adjuvantconveyorcheck;
    ...
    public ConveyorEntity(Segment segment, long x, long y, long z, ushort cube, byte flags, ushort lValue, bool lbLoadedFromDisk) : base(eSegmentEntity.Conveyor, SpawnableObjectEnum.Conveyor, x, y, z, cube, flags, lValue, Vector3.zero, segment)
    {
		...
		*/
            ...
            lValue == (ushort)eConveyorType.eTransportPipe)
        {
            ...
            {
                //Update it's own mesh on placement acourding to it's enviroment
                AutoTurn();
                //Update conveyor mesh in front of this one
                CheckAdjacentConveyor();
            }
        }
        else if (lValue == (ushort)eConveyorType.eConveyorSlopeUp && WorldScript.mbIsServer)
        {
            //Update conveyor mesh in front of this one
            CheckAdjacentConveyor();
        }
    ...
    public override void LowFrequencyUpdate()
    {
        ...
        else if (autoturncheck > 0)
        {
            autoturncheck -= 1;
            //Debug.Log("decrement update autoturn " + autoturncheck.ToString());
        }

        if (adjuvantconveyorcheck == 1)
        {
            adjuvantconveyorcheck = 0;
            OnRotate();
        }
        else if (adjuvantconveyorcheck > 0)
        {
            adjuvantconveyorcheck -= 1;
            //Debug.Log("decrement update autoturn " + adjuvantconveyorcheck.ToString());
        }
    ...
    public override void UnityUpdate()
    {
        ...
        if (!mbLinkedToGO)
        {
            ...
            else
            {
                ...
                mnUpdates = UnityEngine.Random.Range(0, 60);//ensure staggered updates
    ...
    {
        Vector3 directionVector = mForwards;
        if (mObjectType == SpawnableObjectEnum.Conveyor_SlopeUp)
        {
            directionVector += mUp;
        }
        long num = (long)Mathf.Round(directionVector.x);
        long num2 = (long)Mathf.Round(directionVector.y);
        long num3 = (long)Mathf.Round(directionVector.z);
        long x = mnX + num;
        long y = mnY + num2;
        long z = mnZ + num3;
            ...
            case 513:
                {
                    ...
                    if (conveyorEntity == null)
                    {
                        ...
                        adjuvantconveyorcheck = 2;
                        break;
                    }
                    conveyorEntity.autoturncheck = 2;

                    float num4 = Vector3.Dot(mForwards, conveyorEntity.mForwards);
                    if (!(num4 < -0.9f))
                    {
                        break;
                    }
    ...
    private void OnlyUpwards()
    {
        Debug.Log("Stinosko Located belt placement!");
        Vector3 vector3 = new Vector3(0, 1, 0);
        long fx = (long)Mathf.Round(this.mForwards.x);
        long fy = (long)Mathf.Round(this.mForwards.y);
        long fz = (long)Mathf.Round(this.mForwards.z);
        long ux = (long)Mathf.Round(this.mUp.x);
        long uy = (long)Mathf.Round(this.mUp.y);
        long uz = (long)Mathf.Round(this.mUp.z);

        if (fy < -0.5 || fy > 0.5)
        {
            Debug.Log("Stinosko Located belt placement up down!");
            Vector3 rotateto = new Vector3(-ux, 0, -uz);
            RotateConveyorTo(rotateto);
        }
        else
        {
            RotateConveyorTo(this.mForwards);
        }
    }
    ...
    private void AutoTurn()
    {
        ...
        vectors.Add(-this.mForwards);
        vectors.Add(this.mLeft);
        vectors.Add(this.mRight);
        vectors.Add(-this.mUp + this.mLeft);
        vectors.Add(-this.mUp + this.mRight);
        vectors.Add(-this.mUp - this.mForwards);

        List<int> flags = new List<int>();
        if (mValue == 0 || mValue == 19 || mValue == 18)
        {
            Debug.Log("Im regular auto turnable at X" + mnX.ToString() + " Y" + mnY.ToString() + " Z" + mnZ.ToString() + " ");
            flags.Add(0);
            flags.Add(18);
            flags.Add(19);
        }
        else if (mValue == 11 || mValue == 17 || mValue == 16)
        {
            Debug.Log("Im basic auto turnable at X" + mnX.ToString() + " Y" + mnY.ToString() + " Z" + mnZ.ToString() + " ");
            flags.Add(11);
            flags.Add(16);
            flags.Add(17);
        }
        ...
        {
            Debug.Log("Im not auto turnable at X" + mnX.ToString() + " Y" + mnY.ToString() + " Z" + mnZ.ToString() + " ");
            return;
        }
        ...
        {
            Debug.Log("Auto turnable check: " + j.ToString());
            num = (long)Mathf.Round(vectors[j].x);
            num2 = (long)Mathf.Round(vectors[j].y);
            num3 = (long)Mathf.Round(vectors[j].z);
            x = mnX + num;
            y = mnY + num2;
            z = mnZ + num3;
            Segment segment = AttemptGetSegment(x, y, z);
            if (segment == null)
            {
                Debug.Log("Missing segment - edge of loaded segment?");
                continue;
                ...
                case 513:
                    {
                        ...
                        if (conveyorEntity == null)
                        {
                            Debug.Log("Missing neirby conveyor - was it made in the last frame?");
                            //Since there was a conveyor nearby that was initialised, i'm rerunning this function next low frequency update
                            autoturncheck = 2;
                            continue;
                        }

                        if (j > 2 && conveyorEntity.mValue != (ushort)13)
                        {
                            Debug.Log("This is not a slope up");
                            continue;
                        }

                        if (conveyorEntity.mForwards == this.mForwards && (j == 0 || j == 5))
                        {
                            Debug.Log("Belt is at backside");
                            backside = true;
                        }
                        else if (conveyorEntity.mForwards == this.mLeft && (j == 2 || j == 4))
                        {
                            Debug.Log("Belt is at rightside");
                            //conveyor is at right side offloading into this :-)
                            rightside = true;
                        }
                        else if (conveyorEntity.mForwards == this.mRight && (j == 1 || j == 3))
                        {
                            Debug.Log("Belt is at leftside");
                            leftside = true;
                        }
                        break;
                    }
        ...
        if (backside && mValue != (ushort)flags[0])
        {
            BuildRotateFromEntityReplace(this.mSegment, this.mnX, this.mnY, this.mnZ, eCubeTypes.Conveyor, (ushort)flags[0], mFlags);
            Debug.Log("Straight is needing!");
            
        }
        else if (leftside && !rightside && mValue != (ushort)flags[2]  && !backside)
        {
            Debug.Log("left turn needing!");
            
            BuildRotateFromEntityReplace(this.mSegment, this.mnX, this.mnY, this.mnZ, eCubeTypes.Conveyor, (ushort)flags[2], mFlags);
        }
        else if (!leftside && rightside && mValue != (ushort)flags[1] && !backside)
        {
            Debug.Log("right turn needing!");
            BuildRotateFromEntityReplace(this.mSegment, this.mnX, this.mnY, this.mnZ, eCubeTypes.Conveyor, (ushort)flags[1], mFlags);
        }
        else if (leftside && rightside && mValue != (ushort)flags[0])
        {
            Debug.Log("No direct connected belts? Placing just a straight as default?");
            BuildRotateFromEntityReplace(this.mSegment, this.mnX, this.mnY, this.mnZ, eCubeTypes.Conveyor, (ushort)flags[0], mFlags);
        }
        else
        {
            Debug.Log("auto turnable, didn't do anything, mesh was already correctly I hope :-D");
        }
    }

    public bool BuildRotateFromEntityReplace(Segment segment, long x, long y, long z, ushort leType, ushort leValue, byte leFlags)
    {
        bool flag = !WorldScript.mbIsServer;
        bool result;
        if (flag)
        {
            result = false;
        ...
        {
            bool mbIsEmpty = segment.mbIsEmpty;
            if (mbIsEmpty)
            {
            ...
            buildOrder.x = (byte)num;
            buildOrder.y = (byte)num2;
            buildOrder.z = (byte)num3;
            buildOrder.type = leType;
            buildOrder.val = leValue;
            buildOrder.flag = leFlags;
            buildOrder.command = (byte)NetworkBuildCommand.Replace;
            bool flag2 = segment.QueueBuildOrder(ref buildOrder);
            if (flag2)
            {
                WorldScript.instance.mNodeWorkerThread.KickNodeWorkerThread();
                NetworkManager.instance.QueueBuildOrder(segment, ref buildOrder);
                result = true;
            }
            else
            {
    ...
    public void OnRotate()
    {
        ...
        for (int j = 0; j < vectors.Count; j++)
        {
            Debug.Log("after rotation Auto turnable check neibours: " + j.ToString());
            num = (long)Mathf.Round(vectors[j].x);
            num2 = (long)Mathf.Round(vectors[j].y);
            num3 = (long)Mathf.Round(vectors[j].z);
            x = mnX + num;
                ...
                case 513:
                    {
                        ...
                        if (conveyorEntity == null)
                        {
                            Debug.Log("Conveyor is newly placed last frame");
                            //Since there was a conveyor infront that was initialised last frame, i'm rerunning this function next low frequency update
                            adjuvantconveyorcheck = 2;
                        }
                        //Since this conveyor was initialised this frame, i'm rerunning the autoturn function next low frequency update fro that belt
                        conveyorEntity.autoturncheck = 2;
